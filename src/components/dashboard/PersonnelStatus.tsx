...
const Guard = ({ name, role, location, status, checkInTime }: GuardProps) => {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-lg bg-secondary/30 hover:bg-secondary/50 transition-colors">
      <div className="relative flex-shrink-0">
        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-gradient-to-br from-primary/20 to-accent/20 border border-primary/30 flex items-center justify-center">
          <User className="w-4 h-4 sm:w-5 sm:h-5 text-primary" />
        </div>
        <div
          className={cn(
            "absolute -bottom-1 -right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full border-2 border-card",
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <span className={cn("text-xs hidden sm:inline", statusConfig[status]?.textColor)}>
    ...
    <div className="glass-card rounded-xl border border-border/50 overflow-hidden">
      <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-border/50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm sm:text-base text-foreground">Active Personnel</h3>
        </div>
      ...
      <div className="p-3 sm:p-4 space-y-2 max-h-[360px] overflow-y-auto">
        {loading && <p className="text-xs sm:text-sm text-muted-foreground">Loading personnel...</p>}

        {!loading && guards.length === 0 && (
          <p className="text-xs sm:text-sm text-muted-foreground">No personnel found.</p>
        )}
